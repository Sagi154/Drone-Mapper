...
neighbour may be `NotMapped`, `Occupied`, or `OutOfBounds`. When
`drone_radius` is at least one grid step, a grid-adjacent `NotMapped` cell is
always non-passable, so frontiers mark the edge of confirmed-empty space.
The start cell is never treated as the goal; BFS only stops on frontiers
reached after at least one grid step.
